  ...
  return (
    <div className="glass-card p-5 h-full">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-lg font-bold text-foreground">AI Filters</h2>
      </div>

      <div className="space-y-6">
        {filterSections.map((section) => (
          <div key={section.title}>
            <h3 className="text-sm font-semibold text-muted mb-3 uppercase tracking-wider">{section.title}</h3>
            <div className="flex flex-col gap-2">
              {section.options.map((option) => (
                <label key={option} className="flex items-center gap-3 cursor-pointer group">
                  <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${section.selected.includes(option) ? 'bg-accent-blue border-accent-blue' : 'border-white/20 bg-overlay group-hover:border-accent-blue'}`}>
                    {section.selected.includes(option) && <Check size={12} className="text-foreground" />}
                  </div>
                  <input 
                    type="checkbox" 
                    className="hidden" 
                  ...
                  />
                  <span className="text-sm text-muted group-hover:text-foreground transition-colors">{option}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <div>
          <h3 className="text-sm font-semibold text-muted mb-3 uppercase tracking-wider">Min Salary (INR)</h3>
          ...
          <input 
            ...
            className="w-full h-2 bg-overlay-hover rounded-lg appearance-none cursor-pointer accent-accent-blue"
          />
        </div>
      </div>
    </div>
