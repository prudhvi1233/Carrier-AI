      ...
      <div className="flex-1 relative">
        ...
        <div className="flex flex-col gap-6 relative z-10">
          {timeline.map((event, idx) => (
            ...
            >
              <div className="w-8 h-8 rounded-full bg-secondary border-2 border-white/10 flex items-center justify-center shrink-0 shadow-lg group-hover:border-white/30 transition-colors z-10">
                {getIcon(event.type)}
              </div>
              <div className="flex flex-col pt-1 w-full min-w-0">
                ...
                <span className="text-xs text-gray-400 truncate mt-0.5">{event.resume}</span>
              </div>
            </motion.div>
          ))}
        </div>
