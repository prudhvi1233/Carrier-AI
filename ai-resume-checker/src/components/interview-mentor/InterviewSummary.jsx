...
export default function InterviewSummary({ session, onBack }) {
  ...
  if (!session || !session.feedback) {
    return <div className="text-white p-8">No feedback available.</div>;
  }
  ...
  return (
    <div className="glass-card p-6 md:p-8 space-y-8 h-full overflow-y-auto custom-scrollbar print:bg-white print:text-black">
      ...
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6 print:border-black/10">
        <div>
          <h2 className="text-2xl font-bold text-white print:text-black">Interview Performance Report</h2>
          <p className="text-gray-400 print:text-gray-600">Detailed AI analysis of your mock interview session.</p>
        </div>
      ...
      <div className="bg-black/20 p-6 rounded-2xl border border-white/5 print:bg-gray-100 print:border-gray-300">
        <h3 className="text-white print:text-black font-bold mb-2">Final Summary ({feedback.hiring_recommendation})</h3>
        <p className="text-gray-300 print:text-gray-800 text-sm leading-relaxed">{feedback.final_summary}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 bg-gradient-to-br from-accent-blue/10 to-accent-purple/10 border border-accent-blue/20 rounded-2xl p-6 flex flex-col items-center justify-center text-center print:bg-none print:border-gray-300">
          <h3 className="text-sm font-bold text-gray-400 print:text-gray-600 uppercase tracking-wider mb-4">Overall Score</h3>
          <div className="relative w-32 h-32 flex items-center justify-center mb-4">
            ...
            <span className="text-4xl font-black text-white print:text-black">{report.overallScore}</span>
          </div>
        </div>

        <div className="md:col-span-1 bg-black/40 border border-white/10 rounded-2xl p-4 print:bg-white print:border-gray-300">
          <h3 className="text-sm font-bold text-gray-400 print:text-gray-600 uppercase tracking-wider mb-2 text-center">Skill Breakdown</h3>
          <SkillRadar data={report.skillRadar} />
        </div>

        <div className="md:col-span-1 bg-black/40 border border-white/10 rounded-2xl p-4 print:bg-white print:border-gray-300">
          <h3 className="text-sm font-bold text-gray-400 print:text-gray-600 uppercase tracking-wider mb-2 text-center">Question Trend</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-green-500/5 border border-green-500/20 rounded-2xl p-6 print:bg-white print:border-green-300">
            ...
            <ul className="space-y-3">
              {report.strengths.map((str, i) => (
                <li key={i} className="flex items-start gap-2 text-gray-300 print:text-gray-800 text-sm">
        ...
        <div className="space-y-4">
          <div className="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-6 h-full print:bg-white print:border-blue-300">
            ...
            <div className="space-y-4">
              <div>
                <h4 className="text-xs font-bold text-gray-400 print:text-gray-500 uppercase tracking-wider mb-2">Topics to Study</h4>
              ...
              <div>
                <h4 className="text-xs font-bold text-gray-400 print:text-gray-500 uppercase tracking-wider mb-2">Summary</h4>
      ...
      {/* Transcript Replay */}
      <div className="bg-black/40 border border-white/10 rounded-2xl overflow-hidden print:bg-white print:border-gray-300">
        <button 
          onClick={() => setShowTranscript(!showTranscript)}
          className="w-full p-4 flex items-center justify-between text-left hover:bg-white/5 transition-colors print:hidden"
        >
          <div className="flex items-center gap-2 text-white font-bold">
            <MessageSquare size={18} className="text-accent-purple" />
            Interview Transcript Replay (Detailed Metrics)
          </div>
          {showTranscript ? <ChevronUp size={20} className="text-gray-400" /> : <ChevronDown size={20} className="text-gray-400" />}
        </button>
        
        {/* Force show transcript on print */}
        <div className={`p-6 border-t border-white/5 space-y-8 bg-black/20 print:bg-white print:block ${showTranscript ? 'block' : 'hidden'}`}>
          <h3 className="hidden print:block text-xl font-bold mb-4">Detailed Transcript & Metrics</h3>
          {session.answers.map((ans, idx) => (
            <div key={idx} className="space-y-3 pb-6 border-b border-white/5 print:border-gray-200 last:border-0">
              <div className="bg-white/5 print:bg-gray-50 p-4 rounded-xl border border-white/5 print:border-gray-200">
                ...
                <p className="text-gray-300 print:text-black text-sm">{session.questions[idx]?.question}</p>
              </div>
              ...
              {ans.evaluation && (
                <div className="ml-4 md:ml-8 mt-2 space-y-3">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    <div className="bg-black/30 print:bg-gray-100 p-2 rounded-lg flex flex-col items-center justify-center border border-white/5 print:border-gray-300">
                       <span className="text-xs text-gray-400 print:text-gray-600">STAR Method</span>
                       <span className="font-bold text-white print:text-black">{ans.evaluation.star_method_usage || 0}/100</span>
                    </div>
                    <div className="bg-black/30 print:bg-gray-100 p-2 rounded-lg flex flex-col items-center justify-center border border-white/5 print:border-gray-300">
                       <span className="text-xs text-gray-400 print:text-gray-600">Grammar</span>
                       <span className="font-bold text-white print:text-black">{ans.evaluation.grammar_score || 0}/100</span>
                    </div>
                    <div className="bg-black/30 print:bg-gray-100 p-2 rounded-lg flex flex-col items-center justify-center border border-white/5 print:border-gray-300">
                       <span className="text-xs text-gray-400 print:text-gray-600">Completeness</span>
                       <span className="font-bold text-white print:text-black">{ans.evaluation.completeness || 0}/100</span>
                    </div>
                    <div className="bg-black/30 print:bg-gray-100 p-2 rounded-lg flex flex-col items-center justify-center border border-white/5 print:border-gray-300">
                       <span className="text-xs text-gray-400 print:text-gray-600">Tech Accuracy</span>
                       <span className="font-bold text-white print:text-black">{ans.evaluation.technical_accuracy || 0}/100</span>
                    </div>
                  </div>
      ...
      <div className="flex justify-center pt-4 border-t border-white/10 print:hidden">
        <button 
          onClick={onBack}
