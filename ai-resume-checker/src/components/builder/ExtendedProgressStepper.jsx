...
export default function ExtendedProgressStepper({ steps, currentStep, onStepClick }) {
  ...
  return (
    <div className="w-full bg-overlay border-b border-border flex flex-col">
      <div className="flex items-center justify-between px-6 py-2 border-b border-border bg-overlay">
        <span className="text-sm font-bold text-foreground uppercase tracking-wider">Step {currentStep + 1} of {steps.length}</span>
            ...
            >
              <div 
                className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all shadow-lg ${
                  isCompleted ? 'bg-accent-blue text-foreground shadow-accent-blue/20' : 
                  isActive ? 'bg-secondary border-2 border-accent-blue text-accent-blue scale-110 shadow-accent-blue/10' : 
                  'bg-secondary border-2 border-border text-muted'
                }`}
              >
      ...
      <div className="h-1 bg-overlay w-full">
        <div 
