...
export default function AchievementsStep({ data, onUpdate }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="mb-2">
        <h2 className="text-3xl font-bold text-foreground mb-2">Achievements & Awards</h2>
        <p className="text-muted">Showcase your notable accomplishments, awards, or publications.</p>
      </div>
      ...
      <div className="glass-card p-6 border-accent-blue/30 bg-accent-blue/5 flex flex-col md:flex-row gap-6 items-center justify-between">
        <div>
          <h3 className="text-foreground font-bold mb-1">Make them impactful</h3>
          <p className="text-sm text-muted">AI can help you rewrite your achievements to emphasize metrics and results.</p>
        </div>
        <button 
        ...
        <AnimatePresence initial={false}>
          {data.map((a, idx) => (
            <motion.div 
              ...
              exit={{ opacity: 0, x: 20 }}
              className="flex items-start gap-3 bg-overlay p-2 rounded-xl border border-border"
            >
              ...
              <textarea 
                ...
                rows={2}
                className="flex-1 bg-transparent border-none focus:outline-none text-foreground py-2 resize-none"
              />
              <button 
