  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="mb-6">
        ...
        <p className="text-gray-400">List your professional experience. Use our AI to write strong, metric-driven bullet points.</p>
      </div>

      <div className="flex gap-4 p-4 rounded-xl border border-white/10 bg-white/5 mb-4">
        <div className="flex-1">
          <h3 className="text-white font-medium">Do you have work experience?</h3>
          <p className="text-xs text-gray-400">Select No if you are a fresher or student.</p>
        </div>
        <div className="flex gap-2">
          <button 
            onClick={() => onToggleExperience(true)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${hasExperience ? 'bg-accent-blue text-white' : 'bg-black/20 text-gray-400 border border-white/10'}`}
          >
            Yes
          </button>
          <button 
            onClick={() => onToggleExperience(false)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${!hasExperience ? 'bg-red-500/20 border border-red-500/50 text-red-400' : 'bg-black/20 text-gray-400 border border-white/10'}`}
          >
            No
          </button>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4 md:pl-6 pr-8">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center h-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input 
                        ...
                        className="rounded border-white/10 bg-black/20 text-accent-blue focus:ring-accent-blue"
                      />
                  ...
                  <input 
                    ...
                    disabled={ex.current}
                    className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-accent-blue [color-scheme:dark] disabled:opacity-50"
                  />
                </div>

                <div className="flex flex-col gap-2 md:col-span-2 mt-4">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-medium text-gray-400">Description (Bullet points)</label>
                    <button className="text-xs flex items-center gap-1 text-accent-purple hover:text-white transition-colors bg-accent-purple/10 px-2 py-1 rounded">
      ...
      {(!data || data.length === 0) && (
        <div className="p-8 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center text-center">
          <p className="text-gray-400 mb-4">No experience entries added yet.</p>
        </div>
      )}
      ...
      >
        ...
        <motion.div 
          ...
          className="p-8 bg-black/20 border border-white/10 rounded-xl text-center"
        >
          <p className="text-gray-400 text-sm">Experience section will be excluded from your resume.</p>
        </motion.div>
      )}
    </div>
  );
